...
function engineScript(mode: "correct" | "buggy" | "crash" | "garbage" | "empty" | "negative"): string {
  return `
const depth = process.argv[3]
if (${JSON.stringify(mode)} === "crash") process.exit(1)
if (${JSON.stringify(mode)} === "empty") process.exit(0)
if (${JSON.stringify(mode)} === "negative") { console.log("-1"); process.exit(0) }
...
async function writeEngine(mode: "correct" | "buggy" | "crash" | "garbage" | "empty" | "negative"): Promise<string> {
...
test("perft() rejects a negative count (invalid perft output)", async () => {
  const adapter = adapterFor(await writeEngine("negative"))
  await expect(adapter.perft(3, STARTPOS)).rejects.toThrow("did not return a number")
})
